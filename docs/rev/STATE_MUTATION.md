...
The player actor entity (returned by `GetPlayerID()`) is a **positional** component (class 75 in the component hierarchy). When the player is on foot, this entity is parented to the **room** component inside the station/ship.
...
|---------|----------|------|
| `0x14016BBB0` | `GetPositionalOffset` | Exported API — checks class 75, calls `GetRelativeTransform` |
...
2. Checks entity is class 75 (positional) — player entity qualifies

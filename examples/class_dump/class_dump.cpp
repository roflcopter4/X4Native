...
static consteval size_t operator""_ZU(unsigned long long n)
{
...
struct PropChangeTypeEntry {
    char const *name; // short lowercase name ("killed", "attacked", etc.)
    uint32_t    type_id;
    uint32_t    _pad;
};
...
static std::pair<PropChangeTypeEntry const *, uint32_t>
find_propchange_type_table(uintptr_t rdata_start, uintptr_t rdata_end, uintptr_t exe_base)
{
    ...
    for (uintptr_t addr = rdata_start; addr + 16_ZU * 100_ZU <= rdata_end; addr += 8) {
        auto *candidate = reinterpret_cast<PropChangeTypeEntry const *>(addr);

        // Quick check: first entry should have type_id 0
        if (candidate[0].type_id != 0)
            continue;

        // Count sequential type_ids and valid string entries
        uint32_t sequential    = 0;
        uint32_t valid_strings = 0;
        for (uint32_t i = 0; i < 600 && addr + (i + 1_ZU) * 16_ZU <= rdata_end; ++i) {
            auto &e = candidate[i];
            if (e.type_id != i)
                break;
            sequential++;
        ...
        if (sequential >= 500 && valid_strings >= 400) {
            static constexpr char const *known_names[] = {
                "attacked",       "killed",   "destroyed",        "damaged",          "buildfinished",
                "buildcancelled", "activate", "dockingrequested", "aiordercancelled", "airlockcycled",
            };
            uint32_t hits = 0;
            for (uint32_t i = 0; i < sequential && i < 600; i++) {
                char const *n    = candidate[i].name;
                auto        sptr = reinterpret_cast<uintptr_t>(n);
                if (sptr < exe_base || sptr >= exe_base + 0x20000000)
                    continue;
                for (char const *k : known_names) {
                    if (strcmp(n, k) == 0) {
                        ++hits;
                        break;
                    }
...
static std::string extract_event_class_name(char const *decorated)
{
    auto name = std::string_view(decorated);
...
static void dump_event_types(fs::path const &base_path)
{
    ...
    for (WORD i = 0; i < nt->FileHeader.NumberOfSections; ++i) {
        if (strncmp(reinterpret_cast<char *>(sections[i].Name), ".rdata", 6) == 0) {
    ...
    if (table && table_count > 0) {
        ...
        x4n::log::info("class_dump: PropertyChange type table — {} entries, {} with names", table_count, named);
    } else {
    ...
    for (uintptr_t addr = rdata_start; addr + 24 <= rdata_end; addr += 4) {
        ...
        if (col[0] != 1)
            continue;
        if (col[5] != static_cast<uint32_t>(addr - exe_base))
            continue;
        cols_checked++;

        auto  td_addr = exe_base + static_cast<uint32_t>(col[3]);
        auto *td_name = reinterpret_cast<char const *>(td_addr + 16);
        if (!strstr(td_name, "Event@U@@"))
            continue;
        ...
        for (uintptr_t vt_scan = rdata_start; vt_scan + 8 <= rdata_end; vt_scan += 8) {
            if (*reinterpret_cast<uintptr_t *>(vt_scan) == addr) {
                ...
                auto *fn        = static_cast<uint8_t *>(vtable[1]);
                if (fn[0] == 0xB8 && fn[5] == 0xC3) {
                    uint32_t type_id      = *reinterpret_cast<uint32_t *>(fn + 1);
                    rtti_entries[type_id] = {extract_event_class_name(td_name), vtable_va - exe_base};
                }
                break;
            }
        }
    ...
    for (uintptr_t addr = rdata_start; addr + 16_ZU <= rdata_end; addr += 8) {
        auto str_ptr = *reinterpret_cast<uintptr_t *>(addr);
        auto desc_id = *reinterpret_cast<uint32_t *>(addr + 8);

        if (str_ptr < rdata_start || str_ptr >= rdata_end)
            continue;
        auto *str = reinterpret_cast<char const *>(str_ptr);
        if (strncmp(str, "event_", 6) != 0)
            continue;
        if (desc_id < 1000 || desc_id > 2500)
            continue;
        ...
        if (!valid || len < 8 || len > 70)
            continue;
        descid_to_xsd[desc_id] = std::string(str, len);
    }

    x4n::log::info("class_dump: found {} unique event_* descriptor entries", descid_to_xsd.size());

    // ---- Merge and write CSV ----
    std::set<uint32_t> all_ids;
    for (auto const &[id, _] : short_names)
        all_ids.insert(id);
    for (auto const &[id, _] : rtti_entries)
        all_ids.insert(id);

    {
        auto f = std::ofstream(base_path / "event_type_ids.csv");
        if (!f) {
    ...
    x4n::log::info("class_dump: merged {} event types → event_type_ids.csv", all_ids.size());
}
...
static void on_game_loaded()
{
    ...
    static constexpr uint32_t MAX_PAIRS = 10000;
    std::vector<ComponentClassPair> pairs(MAX_PAIRS);
    uint32_t filled = g->GetComponentClassMatrix(pairs.data(), MAX_PAIRS);
    if (filled == 0) {
        x4n::log::warn("class_dump: GetComponentClassMatrix returned 0 pairs — is the game world fully loaded?");
        return;
    }
    pairs.resize(filled);
    ...
    for (auto const &p : pairs) {
        if (p.class1name && p.class1name[0])
            ids.insert({p.class1id, p.class1name});
        if (p.class2name && p.class2name[0])
            ids.insert({p.class2id, p.class2name});
    }

    auto const base = fs::path(x4n::path());
    ...
    {
        auto f = std::ofstream(base / "class_ids.csv");
        if (!f) {
            x4n::log::error("class_dump: could not open {}class_ids.csv for writing", base.string());
            return;
        }
        f << "id,name\n";
        for (auto const &[id, name] : ids)
            f << id << ',' << name << '\n';
    }
    ...
    {
        auto f = std::ofstream(base / "class_matrix.csv");
        if (!f) {
            x4n::log::error("class_dump: could not open {}class_matrix.csv for writing", base.string());
            return;
        }

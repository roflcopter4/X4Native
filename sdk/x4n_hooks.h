...
struct HookControl {
    ...
    template <typename T>
    void set_result(T const &val)
    {
        if (ctx_->result)
            *static_cast<T *>(ctx_->result) = val;
        skip_original = 1;
    }

    explicit HookControl(X4HookContext *ctx)
        : skip_original(ctx->skip_original),
          ctx_(ctx)
    {}
...
inline FuncNameEntry const g_func_names[] = {
#include "x4_game_func_list.inc"
#include "x4_internal_func_list.inc"
};
#undef X4_FUNC

inline char const *name_from_offset(size_t off)
{
    for (FuncNameEntry const &e : g_func_names)
        if (e.offset == off)
            return e.name;
    return nullptr;
}
...
size_t get_offset()
{
    static size_t const off = [] {
        X4GameFunctions dummy = {};
        auto *member = &(dummy.*MPtr);
        return static_cast<size_t>(reinterpret_cast<char *>(member) - reinterpret_cast<char *>(&dummy));
    }();
    return off;
}
...
{
    if (state<MPtr>::detour_installed)
        return true;
...
template <auto MPtr, typename FnPtr>
struct detour_selector;
...
template <typename UserFn, typename FnPtr>
struct before_adapter;
...
{
    using MemberType = detail::member_type<decltype(MemberPtr)>::type;
    ...
    if (!detail::install_detour<MemberPtr>(detour))
        return -1;

    X4HookCallback raw_cb = detail::before_adapter<FpType, MemberType>::get();

    return ::x4n::detail::g_api->hook_before(
        detail::get_name<MemberPtr>(), raw_cb,
        reinterpret_cast<void *>(fp), ::x4n::detail::g_api
    );
}

...
inline GameClass game_class(X4Component const *comp)
{
    if (!comp || !comp->vtable)
        return static_cast<GameClass>(GAMECLASS_SENTINEL);
    using Fn = uint32_t(*)(void const *);
    auto fn  = static_cast<Fn *>(comp->vtable)[detail::offsets()->vtable_get_class_id];
    return static_cast<GameClass>(fn ? fn(comp) : GAMECLASS_SENTINEL);
}

/// IS-A check via vtable IsOrDerivedFromClassID slot.
inline bool is_a(X4Component const *comp, GameClass cls)
{
    if (!comp || !comp->vtable)
        return false;
    using Fn = bool(*)(void const *, uint32_t);
    auto fn  = static_cast<Fn *>(comp->vtable)[detail::offsets()->vtable_is_derived_class];
...
{
    auto *g = game();
    if (!g || !g->ComponentRegistry_Find)
        return nullptr;
    auto *reg = *static_cast<X4ComponentRegistry **>(detail::offsets()->component_registry);
    if (!reg)
        return nullptr;
    return static_cast<X4Component *>(g->ComponentRegistry_Find(reg, id, 4));
}
...
inline char const *get_class_name(uint64_t id)
{
    auto *g = game();
    if (!g || !g->GetComponentClass)
        return "";
...
#ifdef _MSC_VER
inline char const *get_component_macro(X4Component *component)
{
    if (!component || !component->definition.vtable)
        return nullptr;
    auto *str = static_cast<uint64_t *>(component->definition.GetMacroName());
    if (!str)
        return nullptr;
    // MSVC x64 std::string SSO: data inline at str[0..1] if capacity (str[3]) < 16.
    return str[3] < 16
        ? reinterpret_cast<char const *>(str)
        : reinterpret_cast<char const *>(str[0]);
}
#endif

/// Convenience overload: read macro name by UniverseID.
#ifdef _MSC_VER
inline char const *get_component_macro(uint64_t id)
{
...
inline double get_spawntime(X4Component const *comp)
{
    if (!comp)
        return -1.0;
    double ret;
    memcpy(&ret, reinterpret_cast<uint8_t const *>(comp) + detail::offsets()->container_spawntime, sizeof ret);
    return ret;
}
...
{
    return get_spawntime(find_component(id));
}
...
{
    auto *g = game();
    if (!g || !g->GetContextByClass)
        return 0;
    char const *name = class_name(cls);
...
{
  public:
    explicit ComponentEntity(UniverseID id, GameClass cls)
        : id_(id),
          comp_(find_component(id))
    {
        if (comp_ && !is_a(comp_, cls))
            comp_ = nullptr;
    }

    ND bool valid() const { return comp_ != nullptr; }
    ND UniverseID id() const { return id_; }
    ND X4Component *component() const { return comp_; }
